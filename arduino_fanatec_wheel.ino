...
#define DISPLAY_CRC8_MISMATCH_ON_SERIAL          // once you know the SPI port is mostly receiving data correctly, you can comment out both lines (or use them for easy debugging)
                                                   // Note: while serial messages are being sent, buttons are sometimes released (by the wheelbase firmware). Which in turns means that if you're holding a button, it will flicker randomly instead of staying held if serial messages are being generated. (Even if serial communication is disconnected). So make sure that once everything works properly, you comment these options out.
